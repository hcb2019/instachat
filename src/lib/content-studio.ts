...
const deliverableSectionSchema = z.object({ heading: z.string().trim().min(2).max(100), body: z.string().trim().max(1200), items: z.array(z.string().trim().min(2).max(500)).max(12), practicalTip: z.string().trim().min(3).max(500).optional() });
