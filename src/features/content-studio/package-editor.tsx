...
export function PackageEditor({ project, publicUrl, media }: { project: ContentProject; publicUrl: string; media: InstagramMedia[] }) {
  ...
  return <form action={action} className="studio-package"><input type="hidden" name="id" value={project.id}/>
    ...
    <section className="studio-package-columns"><div className="card studio-message-editor"><p className="eyebrow">Automação</p><h2>Palavra e mensagens</h2><CopyInput name="selectedKeyword" label="Palavra-chave selecionada" value={pack.selectedKeyword}/><div className="studio-keyword-options">{pack.keywordSuggestions.map((item)=><input key={item} type="hidden" name="keywordSuggestions" value={item}/>)}{pack.keywordSuggestions.map((item)=><span key={item}><code>{item}</code><CopyButton value={item}/></span>)}</div><h3>Respostas públicas</h3>{pack.publicReplies.map((item,index)=><CopyTextarea key={index} name="publicReplies" label={`Variação ${index+1}`} value={item} rows={2}/>)}<h3>Mensagens privadas</h3>{pack.dmMessages.map((item,index)=><CopyTextarea key={index} name="dmMessages" label={`Mensagem ${index+1}`} value={item} rows={2}/>)}</div>
      <div className="card studio-deliverable-editor"><p className="eyebrow">Entregável</p><h2>Material que vai para o direct</h2><label><span>Título</span><input name="deliverableTitle" defaultValue={pack.deliverable.title}/></label><label><span>Resumo</span><textarea name="deliverableSummary" defaultValue={pack.deliverable.summary} rows={3}/></label><div className="studio-deliverable-stats"><span><strong>{pack.deliverable.sections.length}</strong> etapas</span><span><strong>{pack.deliverable.templates?.length??0}</strong> modelos copiáveis</span><span><strong>{pack.deliverable.examples?.length??0}</strong> exemplo preenchido</span></div><div className="studio-deliverable-outline">{pack.deliverable.sections.map((section)=><article key={section.heading}><strong>{section.heading}</strong><p>{section.body}</p><span>{section.items.length} ações práticas</span></article>)}</div><a href={publicUrl} target="_blank" rel="noreferrer" className="button button-secondary">Abrir página pública <ExternalLink size={15}/></a></div></section>
    ...
    <footer className="studio-package-actions"><input type="hidden" name="conceptIndex" value={String(project.selectedConceptIndex??0)}/><button className="button button-ghost" formAction={generateContentPackage}><Zap size={15}/>Gerar nova versão completa</button><button className="button button-secondary" disabled={pending}><Save size={15}/>{pending?"Salvando…":"Salvar alterações"}</button><button className="button button-primary" formAction={createAutomationFromProject}><Zap size={15}/>{project.automationId?"Abrir automação":"Criar automação em rascunho"}</button></footer>
  </form>;
}
