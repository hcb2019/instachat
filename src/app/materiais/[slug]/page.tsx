...
export default async function PublicMaterialPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="material-page"><MaterialActions/><article className="material-document">
    ...
    <div className="material-layout"><aside className="material-toc"><strong>Faça nesta ordem</strong><nav aria-label="Passos do material">{flow.length ? flow.map((step, index) => <a href={`#passo-${index + 1}`} key={step.title}><span>{String(index + 1).padStart(2, "0")}</span>{step.title}</a>) : material.sections.map((section, index) => <a href={`#etapa-${index + 1}`} key={section.heading}><span>{String(index + 1).padStart(2, "0")}</span>{section.heading}</a>)}</nav><div><Sparkles size={17}/><span><small>O que ficará pronto</small><strong>{finalArtifact}</strong></span></div></aside>
      <div className="material-content">{flow.length > 0 && material.resultPrompt && material.resultPlaceholder && material.finalApplication ? <>
        ...
        <details className="material-deep-dive"><summary>Preciso entender melhor antes de executar</summary><div>{detailedGuide}</div></details>
      </> : detailedGuide}</div>
