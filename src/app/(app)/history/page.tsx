...
export default async function HistoryPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  return <div className="page-shell"><header className="page-header compact"><div><p className="eyebrow">Auditoria</p><h1>Histórico de execuções</h1><p>Cada comentário, tentativa e resultado em uma trilha verificável.</p></div></header>
    ...
    <div className="history-table"><div className="history-head"><span>Pessoa e comentário</span><span>Automação</span><span>Resposta pública</span><span>DM</span><span>Resultado</span><span>Data</span></div>{runs.map((run) => <div className="history-row" key={run.id}><div><strong>@{run.commenterUsername}</strong><small>“{run.commentText}”</small></div><span>{run.automationName}</span><StatusBadge status={run.publicReplyStatus} /><StatusBadge status={run.dmStatus} /><div><StatusBadge status={run.status} />{run.errorMessage && <small className="error-copy">{run.errorMessage}</small>}</div><span>{formatDate(run.createdAt)}</span></div>)}</div>
  </div>;
}
