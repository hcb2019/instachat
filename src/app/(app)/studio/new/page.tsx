...
export default async function NewStudioProjectPage({ searchParams }: { searchParams: Promise<{ insight?: string; title?: string; topic?: string }> }) {
  ...
  return <div className="page-shell"><header className="page-header compact"><div><p className="eyebrow">Nova campanha</p><h1>Vamos encontrar o melhor ângulo.</h1><p>Você traz a situação. O InstaChat cria três caminhos com hook, material e chamada conectados.</p></div></header>{!isDemoMode && !env.OPENAI_API_KEY && <p className="form-alert">O Estúdio está usando a geração local, baseada somente neste briefing. Configure OPENAI_API_KEY no servidor para liberar a redação completa por IA.</p>}<ContentProjectForm profile={profile} seed={{ title: query.title, topic: query.topic, sourceInsightId: query.insight }} /></div>;
}
