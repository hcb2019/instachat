...
import { createAutomationFromInsight, requestAudienceAnalysis, updateInsight } from "@/features/audience/actions";
...
export default async function RadarPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="page-shell radar-page">
    ...
    {activeInsights.length === 0 ? <EmptyState title="O Radar está aguardando sinais" body="Atualize os comentários e execute uma análise para revelar temas e oportunidades." /> : <>
      ...
      <section className="radar-section studio-section"><div className="studio-title"><div className="studio-mark"><Lightbulb size={22} /></div><div><p className="eyebrow">Estúdio de ideias</p><h2>Sinais transformados em próximos Reels</h2><p>Roteiros sugeridos pela IA. Nada será publicado ou ativado sem a sua revisão.</p></div></div><div className="idea-grid">{data.ideas.map((idea, index) => { const suggestion = idea.contentSuggestion!; return <article className="idea-card" key={idea.id}><div className="idea-number">IDEIA {String(index + 1).padStart(2, "0")}</div><Badge tone="accent">{categoryLabel(idea.category)}</Badge><h3>“{suggestion.hook}”</h3><p className="idea-angle">{suggestion.angle}</p><ol>{suggestion.outline.map((item) => <li key={item}>{item}</li>)}</ol><div className="idea-footer"><div><span>CTA sugerido</span><strong>{suggestion.cta}</strong></div><div><span>Palavra-chave</span><code>{suggestion.keyword}</code></div></div><form action={createAutomationFromInsight}><input type="hidden" name="id" value={idea.id} /><button className="button button-primary" type="submit">Criar rascunho <ArrowRight size={15} /></button></form></article>; })}</div></section>
    </>}
